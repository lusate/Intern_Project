...
public class CommonUtils { 	
	...
	public static boolean checkByteLength(String str, int limitByte){	 
		...
		for (int j = 0; j < string.length; j++) {
			...
			else if (string[j] >= '\u3131' && string[j] <= '\uD787') { ko += 2; }
			else { etc++; }
		}
		int sum = en + ko + etc;
	...
	public static boolean compareDate(String startDate, String endDate){
		...
		if(startDate.length() == 7){
			...
			if(m0.equals("0") || m0.equals("1")){ startDate = startDate.substring(0,6)+"0"+startDate.substring(6); }
			else{ startDate = startDate.substring(0,4)+"0"+startDate.substring(4); }
		} else if(startDate.length() == 6){ startDate = startDate.substring(0, 4) + "0" + startDate.substring(4, 5) + "0" + startDate.substring(5, 6); }
		if(endDate.length() == 7){
			String m0 = endDate.substring(4,5);
			if(m0.equals("0") || m0.equals("1")){ endDate = endDate.substring(0,6)+"0"+endDate.substring(6); }
			else { endDate = endDate.substring(0,4)+"0"+endDate.substring(4); }
		} else if(endDate.length() == 6){ endDate = endDate.substring(0, 4) + "0" + endDate.substring(4, 5) + "0" + endDate.substring(5, 6); }
